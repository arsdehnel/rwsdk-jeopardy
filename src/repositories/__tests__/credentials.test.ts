...
describe('createCredential', () => {
	it('creates a credential with all required fields', async () => {
		...
		const credential = await createCredential(credData, user.id, logger);
	...
	it('creates multiple credentials for same user', async () => {
		...
		const cred1 = await createCredential(createCredentialData(user.id), user.id, logger);
		const cred2 = await createCredential(createCredentialData(user.id), user.id, logger);
	...
	it('creates credentials for different users', async () => {
		...
		const cred1 = await createCredential(createCredentialData(user1.id), user1.id, logger);
		const cred2 = await createCredential(createCredentialData(user2.id), user2.id, logger);
	...
	it('handles unique credentialId constraint', async () => {
		...
		await createCredential(createCredentialData(user.id, { credentialId }), user.id, logger);
...
describe('getCredentialsByUserId', () => {
	...
	it('returns multiple credentials for user', async () => {
		...
		const cred1 = await createCredential(createCredentialData(user.id), user.id, logger);
		const cred2 = await createCredential(createCredentialData(user.id), user.id, logger);
		const cred3 = await createCredential(createCredentialData(user.id), user.id, logger);
	...
	it('returns only credentials for specified user', async () => {
		...
		await createCredential(createCredentialData(user1.id), user1.id, logger);
		const user2Cred = await createCredential(createCredentialData(user2.id), user2.id, logger);
...
describe('getCredentialById', () => {
	...
	it('returns correct credential when multiple exist', async () => {
		...
		await createCredential(createCredentialData(user.id), user.id, logger);
		const target = await createCredential(createCredentialData(user.id), user.id, logger);
		await createCredential(createCredentialData(user.id), user.id, logger);
...
describe('updateCredentialCounter', () => {
	it('updates counter for credential', async () => {
		const user = await createUser('testuser', null, logger);
		const credential = await createCredential(createCredentialData(user.id), user.id, logger);
...
describe('integration: credential lifecycle', () => {
	it('can create, retrieve, and update credential', async () => {
		...
		// Create
		const created = await createCredential(createCredentialData(user.id, { name: 'My Key' }), user.id, logger);
	...
	it('can manage multiple credentials per user', async () => {
		...
		const yubikey = await createCredential(createCredentialData(user.id, { name: 'Yubikey' }), user.id, logger);
		const iphone = await createCredential(createCredentialData(user.id, { name: 'iPhone' }), user.id, logger);
